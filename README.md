...
| [Refresh usage on demand](patches/feat_add_a_refresh_command_that_fetches_usage_now_one_account_at_a_time.patch)             | After a usage endpoint 429, `ls` shows hour-old numbers with no way to ask again. `cswap refresh [account ...]` fetches now, each account on its own, and tells you when the server allows a retry.                                   |
