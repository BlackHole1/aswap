...
| [Open Chrome per account for Claude in Chrome](patches/feat_open_chrome_as_a_stored_account_so_claude_in_chrome_follows_it.patch) | `cswap switch` moves Claude Code to another account, but the Claude in Chrome extension keeps acting as whoever is signed in to claude.ai in the browser, a cookie that is a different credential from the one cswap stores (upstream #256). Swapping that cookie would mean decrypting Chrome's storage and rewriting another extension's state. The patch gives every stored account its own permanent Chrome profile and opens the browser against it with `--user-data-dir`. `cswap chrome 2` opens account 2 in its own browser, a copy of your stock Chrome profile minus its claude.ai cookies: other sites stay signed in, your extensions come along, and after one sign-in there the account and the extension stay put. Each profile is named after its account and gets its own window colour, an account that is already open is brought forward, and `cswap chrome --sync 2` copies the stock profile in again once that browser is closed, keeping the account's login. `chrome.followSwitch` makes `cswap switch` bring the matching browser forward, and `chrome.appPath` points it at Edge, Brave or Chromium instead. |
