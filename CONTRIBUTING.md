...
| `bun run install:all`            | both. Use this to try a change on your machine when both are installed: the `aswap` wheel bundles its own copy of the cswap code, so `install:cswap` alone leaves an installed `aswap` on the old build    |
