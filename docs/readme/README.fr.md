...
| [Ouvrir Chrome par compte pour Claude in Chrome](../../patches/feat_open_chrome_as_a_stored_account_so_claude_in_chrome_follows_it.patch)          | `cswap switch` change le compte de Claude Code, mais l'extension Claude in Chrome continue d'agir comme le compte connecté à claude.ai dans le navigateur, un cookie qui est un identifiant distinct de celui que cswap stocke (upstream #256). Remplacer ce cookie demanderait de déchiffrer le stockage de Chrome et de réécrire l'état d'une autre extension. Le patch donne à chaque compte enregistré un profil Chrome permanent et lance le navigateur dessus avec `--user-data-dir` : `cswap chrome 2` ouvre le compte 2 dans son propre navigateur : une copie du profil Chrome habituel sans ses cookies claude.ai, donc les autres sites restent connectés et les extensions suivent ; connectez-vous à Claude une fois et cela reste ainsi. Chaque profil porte le nom de son compte avec sa propre couleur de fenêtre, un compte déjà ouvert est ramené au premier plan, et `cswap chrome --sync 2` recopie le profil habituel plus tard en gardant la connexion du compte. `chrome.appPath` permet de viser Edge, Brave ou Chromium. |
