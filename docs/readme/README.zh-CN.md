...
| 补丁                                                                                                                                    | 问题                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [跟随活动凭据的真正所有者](../../patches/fix_follow_the_live_credentials_owner_when_the_config_names_another_slot.patch)                | 多个 Claude Code 进程共用 `~/.claude.json`。在 `cswap switch` 之前就已启动的进程，内存里还是原来的账户，会把它写回去 (实测中 Chrome 扩展的 native host 在切换 16 秒后就这么做了)。于是配置里写着账户 B，而凭据存储里放的仍是账户 A 的 token。上游相信这个标签：把 A 当成空闲账户，发出 A 早已被消耗的备份 refresh token，收到 `invalid_grant`，给 A 显示一个假的 "re-login needed"；B 则被喂了 A 的外来 token，永远不会从自己的备份刷新。两个账户都停滞，而界面把责任算在 A 头上。补丁把活动凭据归属到真正的所有者，跨运行记住这个结论，让另一个账户继续从自己的备份刷新，并修复被误判的备份。 |
| [按实际安装的发行版报告版本、检查更新和升级](../../patches/feat_report_check_and_upgrade_the_distribution_this_install_came_from.patch) | 上游把 `claude-swap` 这个发行版名写死在版本号、PyPI 更新检查和 `cswap upgrade` 里。装成 `aswap` 后会导入即崩、每次运行都提示 claude-swap 有新版、升级时装错包。补丁改为读取已安装的元数据，跟随真正提供这份代码的发行版。                                                                                                                                                                                                                                                                                                                                                                      |

每个补丁都是一次提交，提交信息说明上游的问题。完整列表见 [patches/.patches](../../patches/.patches)。

## 构建方式

上游不做 fork。它以 git submodule 的形式原样引入到 `cswap/`，所有改动都以普通的 `git am` 补丁存放在 `patches/` 下，按 `patches/.patches` 列出的顺序依次应用。这正是 [Electron 管理 Node 和 Chromium 补丁的方式](https://github.com/electron/electron/tree/main/patches)：差异保持小而易读，升级上游只是对补丁序列做一次 rebase，而不是 merge。细节、命令和补丁工作流见 [CONTRIBUTING.md](../../CONTRIBUTING.md)。
