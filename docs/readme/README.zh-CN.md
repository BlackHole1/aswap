...
| [按账户打开 Chrome (Claude in Chrome)](../../patches/feat_open_chrome_as_a_stored_account_so_claude_in_chrome_follows_it.patch) | `cswap switch` 切换的是 Claude Code，而 Claude in Chrome 扩展仍以浏览器里登录 claude.ai 的那个账户工作，那是一枚与 cswap 所存凭据不同的 cookie (上游 #256)。要替换这枚 cookie 就得解密 Chrome 的存储并改写另一个扩展的状态。补丁给每个已存账户一个永久的 Chrome profile，并用 `--user-data-dir` 启动浏览器。`cswap chrome 2` 在独立浏览器里打开账户 2，它是默认 Chrome profile 去掉 claude.ai cookie 后的副本：其他网站保持登录，扩展原样带过来，在里面登录一次后账户和扩展都一直保持。每个 profile 以账户命名并有自己的窗口颜色，账户已打开时则把它的浏览器带到前台，关闭该账户的浏览器后 `cswap chrome --sync 2` 可从默认 profile 重新同步并保留该账户的登录。`chrome.followSwitch` 让 `cswap switch` 顺带把对应的浏览器带到前台，`chrome.appPath` 可改为指向 Edge、Brave 或 Chromium。 |
