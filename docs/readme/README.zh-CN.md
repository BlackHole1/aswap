...
| [按账户打开 Chrome (Claude in Chrome)](../../patches/feat_open_chrome_as_a_stored_account_so_claude_in_chrome_follows_it.patch) | `cswap switch` 切换的是 Claude Code，而 Claude in Chrome 扩展仍以浏览器里登录 claude.ai 的那个账户工作，那是一枚与 cswap 所存凭据不同的 cookie (上游 #256)。要替换这枚 cookie 就得解密 Chrome 的存储并改写另一个扩展的状态。补丁给每个已存账户一个永久的 Chrome profile，并用 `--user-data-dir` 启动浏览器：`cswap chrome 2` 在独立浏览器里打开账户 2，扩展在其中安装并登录一次后一直保持；账户已打开时则把它的浏览器带到前台。`chrome.appPath` 可改为指向 Edge、Brave 或 Chromium。 |
