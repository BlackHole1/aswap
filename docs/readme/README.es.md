...
| [Abrir Chrome por cuenta para Claude in Chrome](../../patches/feat_open_chrome_as_a_stored_account_so_claude_in_chrome_follows_it.patch)                | `cswap switch` cambia Claude Code de cuenta, pero la extensión Claude in Chrome sigue actuando como quien tenga la sesión iniciada en claude.ai en el navegador, una cookie que es una credencial distinta de la que guarda cswap (upstream #256). Sustituir esa cookie exigiría descifrar el almacenamiento de Chrome y reescribir el estado de otra extensión. El parche da a cada cuenta guardada un perfil de Chrome permanente y abre el navegador sobre él con `--user-data-dir`: `cswap chrome 2` abre la cuenta 2 en su propio navegador: una copia del perfil de Chrome habitual sin sus cookies de claude.ai, así que los demás sitios siguen con la sesión iniciada y las extensiones vienen incluidas; inicia sesión en Claude allí una vez y así se queda. Cada perfil lleva el nombre de su cuenta y un color de ventana propio, y una cuenta ya abierta se trae al frente. `chrome.appPath` permite apuntar a Edge, Brave o Chromium. |
