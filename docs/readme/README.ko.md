...
| [사용량 즉시 새로 고침](../../patches/feat_add_a_refresh_command_that_fetches_usage_now_one_account_at_a_time.patch)                    | 사용량 API가 429를 반환하면 `ls`는 한 시간 전 수치를 보여 주고 다시 가져올 방법이 없습니다. `cswap refresh [계정 ...]`은 지금 바로 가져오고, 계정마다 독립적으로 동작하며, 제한 중이면 서버가 재시도를 허용하는 시각을 알려 줍니다.            |
